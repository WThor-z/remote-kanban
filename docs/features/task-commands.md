...
- 新建任务必须绑定 `projectId`，未选择项目时后端返回 `422`。
- 任务执行时由服务端根据项目绑定自动解析 `cwd` 与目标网关主机，客户端不再上送工作目录。
- 若项目不存在返回 `404`；项目绑定主机不可用时返回 `409`。
...
- 触发任务执行时，确认请求体仅包含执行参数（如 `agentType`、`baseBranch`、`model`），`cwd` 由后端按项目注入。
