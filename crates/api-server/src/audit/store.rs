...
fn matches_contains(haystack: &str, filter: Option<&str>) -> bool {
...
impl AuditStore {
    ...
    pub async fn list_paginated(&self, query: &AuditListQuery) -> (Vec<AuditEvent>, bool) {
        ...
        let host_filter = trim_to_none(query.host_id.clone()).map(|value| value.to_lowercase());
        ...
        for event in state.iter().rev() {
            ...
            if !matches_contains(&event.action, action_filter.as_deref()) {
                continue;
            }
            ...
            if let Some(host_id) = host_filter.as_deref() {
                let event_host = event.host_id.as_deref().unwrap_or("").to_lowercase();
